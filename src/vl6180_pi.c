...
void start_als(vl6180 handle) 
{
    write_byte(handle, 0x038, 0x01);
}
...
vl6180 vl6180_initialise_address(int device, int addr){
    ...
    if(setup == 1){
        ...
        //write_byte(handle,0x0014, 0x20); // change to just ALS
...
int get_ALS(vl6180 handle)
{
    int als = 0;
    start_als(handle);
    poll_als(handle);

    als = read_byte(handle, 0x051);
    clear_interrupts(handle);
    return als;
}

void get_ID(vl6180 handle)
{
    int devID = 0;
    devID = read_byte(handle, 0x000);
    printf("\nDevice ID: %x", devID);
}
